...
C_Network::NetworkBase::NetworkBase(const NetAddress& netAddr, uint maxSessionCnt) : _netAddr(netAddr), _iocpHandle(nullptr), _monitoringFlag(false)
{
...
C_Network::NetworkErrorCode C_Network::NetworkBase::Begin()
{
	...
	_monitorThread = std::thread([this]() {this->ProcessMonitoring(); });
...
C_Network::NetworkErrorCode C_Network::NetworkBase::ProcessMonitoring()
{
	while (_monitoringFlag)
	{
		printf("[ Current Connect Session Count : %d ]\n", _sessionMgr->GetCurSessionCount());
